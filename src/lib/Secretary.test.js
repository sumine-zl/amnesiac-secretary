...
describe('Utility functions', () => {
    ...
    describe('base64ToBuffer / bufferToBase64', () => {
        it('round-trips binary data', () => {
            const original = new Uint8Array([0, 1, 255, 128, 64, 32, 16, 8, 4, 2]).buffer;
    ...
    describe('compress / decompress', () => {
        it('round-trips small data', async () => {
            ...
            expect(new Uint8Array(decompressed)).toEqual(new Uint8Array(original));
        });
...
describe('Unlock existing vault / verifyPassphrase', () => {
    ...
    beforeAll(() => {
        ...
        Secretary.reset();
        return Secretary.unlock('testpass').then(() => Secretary.encode()).then((c) => {
            ct = c;
            Secretary.reset();
        });
    }, 30000);
    ...
    it('verifyPassphrase returns true for correct passphrase', async () => {
        Secretary.reset();
        await expect(Secretary.verifyPassphrase('testpass', ct)).resolves.toBe(true);
    });

    it('verifyPassphrase returns false for wrong passphrase', async () => {
        await expect(Secretary.verifyPassphrase('wrong', ct)).resolves.toBe(false);
    });
...
describe('Password generation', () => {
    ...
    it('throws when not unlocked', async () => {
        Secretary.reset();
        await expect(Secretary.generate('id1', 'id2', '1', 20, 91)).rejects.toThrow('Not unlocked');
    });
    ...
    describe('with unlocked vault', () => {
        ...
        it('strength 36 (numbers + lowercase)', async () => {
            ...
            expect(pwd.length).toBe(20);
            expect(allCharsIn(pwd, CHARSET_NUMBER + CHARSET_ALPHABET_L)).toBe(true);
        });

        it('strength 62 (numbers + lower + upper)', async () => {
            const pwd = await Secretary.generate('id1', 'id2', '1', 20, 62);
            expect(pwd.length).toBe(20);
            expect(allCharsIn(pwd, CHARSET_NUMBER + CHARSET_ALPHABET_L + CHARSET_ALPHABET_U)).toBe(true);
        });

        it('strength 91 (numbers + lower + upper + special_29)', async () => {
            const pwd = await Secretary.generate('id1', 'id2', '1', 20, 91);
            expect(pwd.length).toBe(20);
            expect(allCharsIn(pwd, CHARSET_NUMBER + CHARSET_ALPHABET_L + CHARSET_ALPHABET_U + CHARSET_SPECIAL_29)).toBe(true);
        });

        it('strength 94 (includes " \' \\ and more specials, no space)', async () => {
            const pwd = await Secretary.generate('id1', 'id2', '1', 20, 94);
            expect(pwd.length).toBe(20);
            expect(allCharsIn(pwd, CHARSET_NUMBER + CHARSET_ALPHABET_L + CHARSET_ALPHABET_U + CHARSET_SPECIAL_32)).toBe(true);
        ...
        it('strength 95 (includes space)', async () => {
            ...
            expect(pwd.length).toBe(20);
            expect(allCharsIn(pwd, CHARSET_NUMBER + CHARSET_ALPHABET_L + CHARSET_ALPHABET_U + CHARSET_SPECIAL_33)).toBe(true);
        });

        it('output length matches requested length', async () => {
            const lengths = [8, 16, 24, 32];
            for (const len of lengths) {
                const pwd = await Secretary.generate('len', 'test', '1', len, 62);
        ...
        it('throws for invalid strength value', async () => {
            await expect(Secretary.generate('id', 'id', 'r1', 20, 99)).rejects.toThrow('Invalid strength value');
            await expect(Secretary.generate('id', 'id', 'r1', 20, -1)).rejects.toThrow('Invalid strength value');
        });
